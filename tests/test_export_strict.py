...
def test_export_writes_predicted_coordinates_not_sample_defaults(tmp_path: Path) -> None:
    sample = tmp_path / "sample.csv"
    preds = tmp_path / "preds.parquet"
    out = tmp_path / "submission.csv"
    ...
    _write_csv(
        sample,
        [
            {"ID": "A_1", "resname": "A", "resid": 1, "x_1": 0.0, "y_1": 0.0, "z_1": 0.0},
            {"ID": "A_2", "resname": "C", "resid": 2, "x_1": 0.0, "y_1": 0.0, "z_1": 0.0},
        ],
    )
    pl.DataFrame(
        [
            {"ID": "A_1", "resname": "A", "resid": 1, "model_id": 1, "x": 1.25, "y": 2.25, "z": 3.25},
            {"ID": "A_2", "resname": "C", "resid": 2, "model_id": 1, "x": 4.50, "y": 5.50, "z": 6.50},
        ]
    ).write_parquet(preds)

    export_submission_from_long(sample_submission_path=sample, predictions_long_path=preds, out_submission_path=out)
    got = pl.read_csv(out).sort("ID")
    assert got.select("x_1", "y_1", "z_1").to_dict(as_series=False) == {
        "x_1": [1.25, 4.5],
        "y_1": [2.25, 5.5],
        "z_1": [3.25, 6.5],
    }
