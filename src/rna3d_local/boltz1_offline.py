...
def predict_boltz1_offline(
    *,
    ...
    if not out_path.exists():
        raise_error(stage, location, "runner boltz1 nao gerou arquivo de saida", impact="1", examples=[str(out_path)])
    predictions = read_table(out_path, stage=stage, location=location)
    validate_long_predictions(predictions=predictions, targets=targets, n_models=int(n_models), stage=stage, location=location, label="boltz1_predictions")
    ...
    write_json(
        manifest_path,
        {
            ...
            "params": {"n_models": int(n_models), "multimodal_ligand_smiles": True, "entrypoint": cmd},
            "stats": {
